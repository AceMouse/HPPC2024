...
void exchange_ghost_cells(World &world) {
    ...
    MPI_Request request[4]; int tag = 0;
    // MPI_Status stat; 
    
    double send_left[world.latitude] = {0};
    double send_right[world.latitude] = {0};
    double recv_left[world.latitude] = {0};
    double recv_right[world.latitude] = {0};
    
    for (uint64_t i = 0; i < world.latitude; ++i) {
        send_right[i] = world.data[i*world.longitude + world.longitude-2];
        send_left[i] =  world.data[i*world.longitude + 1];  
    }
    
    int left = (mpi_rank + mpi_size - 1) % mpi_size;
    int right = (mpi_rank + 1) % mpi_size;

    MPI_Isend(&send_right, world.latitude, MPI_DOUBLE, right, tag, MPI_COMM_WORLD, &request[0]);
    MPI_Isend(&send_left, world.latitude, MPI_DOUBLE, left, tag, MPI_COMM_WORLD, &request[1]);
    MPI_Irecv(&recv_right, world.latitude, MPI_DOUBLE, right, tag, MPI_COMM_WORLD, &request[2]);
    MPI_Irecv(&recv_left, world.latitude, MPI_DOUBLE, left, tag, MPI_COMM_WORLD, &request[3]);
    ...
    // not communicated yet, only local when doings slab
    for (uint64_t j = 0; j < world.longitude; ++j) {
...
void simulate(uint64_t num_of_iterations, const std::string& model_filename, const std::string& output_filename) {
    ...
    // DONE: compute offsets according to rank and domain decomposition
    ...
    const long int offset_longitude = (mpi_rank * (longitude - 2)) - 1; // -1 because first cell is a ghostcell
    const long int offset_latitude  = -1;
    
    // std::cout << "Rank " << mpi_rank << " out of " << mpi_size << std::endl;
    // std::cout << " longitude: " << longitude << " offset_longitude: " << offset_longitude << std::endl;
    // std::cout << " latitude: " << latitude << " offset_latitude: " << offset_latitude << std::endl;
    // WHAT TO DO, with uneven number of processes? local.longitude probably has to be int??
    ...
    for (uint64_t iteration=0; iteration < num_of_iterations; ++iteration) {
        ...
        for (int p = 0; p < mpi_size; ++p) {
            uint64_t off_lat = -1;
            uint64_t off_long = (p * (longitude - 2)) - 1;
        for (uint64_t i = 1; i < latitude-1; ++i)
        for (uint64_t j = 1; j < longitude-1; ++j) {
