...
void exchange_ghost_cells(World &world) {
    ...
    MPI_Status stat; MPI_Request request = MPI_REQUEST_NULL; int tag = 0;
    int n = world.latitude;
    uint64_t send_left[n] = {0};
    uint64_t send_right[n] = {0};
    uint64_t recv_left[n] = {0};
    uint64_t recv_right[n] = {0};
    ...
    MPI_Isend(&send_right, world.latitude, MPI_UINT64_T, right, tag, MPI_COMM_WORLD, &request);
    MPI_Isend(&send_left, world.latitude, MPI_UINT64_T, left, tag, MPI_COMM_WORLD, &request);
    MPI_Irecv(&recv_right, world.latitude, MPI_UINT64_T, right, tag, MPI_COMM_WORLD, &request);
    MPI_Irecv(&recv_left, world.latitude, MPI_UINT64_T, left, tag, MPI_COMM_WORLD, &request);
...
void simulate(uint64_t num_of_iterations, const std::string& model_filename, const std::string& output_filename) {
    ...
    const uint64_t latitude  = global_world.latitude; // mpi_size) + 2;
    const long int offset_longitude = (mpi_rank * (longitude - 2)) - 1; // -1 because first cell is a ghostcell
    const long int offset_latitude  = 0; //(mpi_rank * (latitude - 2)) - 1;;
